...
def youtube_api(youtuber):
    youtube = DEVELOPER_KEY()
    # 샘플 뽑기
    # 채널 id 가져오기 chaanels로 변경가능한지 해보기
    search_response = youtube.search().list(
        q=youtuber,
        order='relevance',
        part='snippet',
    ...
    pd_profile_data = {'profile_name': profile_name, 'subscriber': subscriber,
                       'video_count': video_count, 'profile_image': profile_image}
    # -------메인부분---------
    ...
    pd_videolist_data = {"video_thumbnail": video_thumbnails_list, "video_title": video_titles_list,
                         "video_id": video_ids_list}
    video_list_df = pd.DataFrame(pd_videolist_data)
    transposed_pd_video_list_data = [pd_videolist_data[key] for key in sorted(pd_videolist_data)]
